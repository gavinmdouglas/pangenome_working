...
per_sp_similarities <- readRDS("/data1/gdouglas/projects/honey_bee/combined_Ellegaard.2019.2020/pandora_running/pandora_out_illumina_all_present/gene_presence_noncore_by_species_sample_similarities.rds")

metrics <-  c("alpha_mle", "jaccard", "simpson")

adonis_summary <- data.frame(matrix(NA, nrow = length(per_sp_similarities) * 3, ncol = 11))
...
adonis_summary$Species <- rep(names(per_sp_similarities), each = 3)
...
for (sp in names(per_sp_similarities)) {
  ...
  for (m in metrics) {
    ...
    sim_matrix <- per_sp_similarities[[sp]][[m]]
    
    sample_metadata_subset <- sample_metadata[rownames(sim_matrix), ]
    
    if (m == "alpha_mle") {
      max_val <- max(sim_matrix, na.rm = TRUE)
      dist_matrix <- max_val - sim_matrix
    } else {
      dist_matrix <- 1 - sim_matrix
    }
    
    dist_matrix[upper.tri(dist_matrix)] <- t(dist_matrix)[upper.tri(dist_matrix)]
    dist_matrix <- as.dist(dist_matrix)
    dist_matrix[diag(dist_matrix)] <- 0
...
adonis_summary_long$Metric[which(adonis_summary_long$Metric == "alpha_mle")] <- "Alpha (MLE)"
...
ggplot(data = adonis_summary_long_no_residual, aes(x = value, y = Species, colour = Metric)) +
  ...
  facet_wrap(. ~ variable) +
  scale_colour_manual(values = c("deepskyblue4", "burlywood2", "chartreuse")) +
...
adonis_summary_w_PCs <- data.frame(matrix(NA, nrow = length(per_sp_similarities) * 3, ncol = 17))
...
adonis_summary_w_PCs$Species <- rep(names(per_sp_similarities), each = 3)
...
adonis_summary_w_PCs_long$Metric[which(adonis_summary_w_PCs_long$Metric == "alpha_mle")] <- "Alpha (MLE)"
