...
# Also make network based only on connections at DBH < 0.1
discover_output_edges_0.1 <- discover_output[which(discover_output$q.value < 0.1), c("gene1", "gene2")]
colnames(discover_output_edges_0.1) <- c("from", "to")

all_unique_nodes_0.1 <- c(discover_output_edges_0.1$from, discover_output_edges_0.1$to) 
all_unique_nodes_0.1 <- all_unique_nodes_0.1[-which(duplicated(all_unique_nodes_0.1))]

nodes_info_0.1 <- data.frame(id = all_unique_nodes_0.1,
                             annot = panaroo_out[all_unique_nodes_0.1, "Annotation"])

nodes_info_0.1$genus <- NA
nodes_info_0.1[grep("Bifidobacterium", nodes_info_0.1$id), "genus"] <- "Bifidobacterium"
nodes_info_0.1[grep("Firm4", nodes_info_0.1$id), "genus"] <- "Firm4"
nodes_info_0.1[grep("Firm5", nodes_info_0.1$id), "genus"] <- "Firm5"
nodes_info_0.1[grep("Gilli", nodes_info_0.1$id), "genus"] <- "Gilliamella"
nodes_info_0.1[grep("Snod", nodes_info_0.1$id), "genus"] <- "Snodgrassella"

net_0.1 <- graph_from_data_frame(d = discover_output_edges_0.1,
                                 vertices = nodes_info_0.1,
                                 directed = FALSE)
net_flattened_0.1 <- ggnetwork(net_0.1)

ggplot(net_flattened_0.1, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges() +
  geom_nodes(aes(color = genus), size = 3) +
  theme_blank()
